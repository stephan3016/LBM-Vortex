...
//Analytical linear velocity profile. Constructor:
template <typename T>
LinearProfile3D<T>::LinearProfile3D(SuperGeometry<T,3>& superGeometry_, int material_, std::vector<T>& maxVelocity_):AnalyticalF3D<T,T>(3), clout(std::cout, "LinearProfile3D"),maxVelocity(maxVelocity_)
{
    olb::Vector<T, 3> min = superGeometry_.getStatistics().getMinPhysR(material_);
    olb::Vector<T, 3> max = superGeometry_.getStatistics().getMaxPhysR(material_);
     for (int iD = 0; iD < 3; iD++){
...
{
    output[0] = betaVel[0]*pow((x[2]-x0[2])/(x[2]+beta[0]),1/alpha[0]);          // velocity only in x-direction
    output[1] = 0;
    output[2] = 0;
    return true;
}
