...
/**
  This functor returns a Linear in z-direction profile for use with a pipe with square shaped cross-section.
*/
template <typename T>
class LinearProfile3D final : public AnalyticalF3D<T,T> {
protected:
    OstreamManager clout;
    olb::Vector<T,3> x0;
    olb::Vector<T,3> x1;
...
public:
    ///Constructor:
    LinearProfile3D(SuperGeometry<T,3>& superGeomtry_, int material__, std::vector<T>& maxVelocity_);
    bool operator()(T output[], const T x[]) override;
};
